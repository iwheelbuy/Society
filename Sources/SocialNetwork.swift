...
private let urlRedirectSimplified = "https://iwheelbuy.github.io/SocialNetwork/index.html"
...
private func getUrlRequestFrom(path: String, parameters: [String: String]) -> URLRequest {
   ...
   guard let httpBody = httpBodyString.data(using: .utf8) else {
      fatalError("Failed to create Data from string: \"\(httpBodyString)\"")
   }
   let httpBodyLength = httpBody.count
...
private func getParametersFrom(request: URLRequest, _ completion: @escaping ([String: String]) -> ()) {
   ...
   default:
      let task = URLSession.shared.dataTask(with: request, completionHandler: { (data: Data?, aaaa, gggg) in
...
public extension SocialNetwork {
   ///
   static func didProceed(url: URL) -> Bool {
   ...
   static func didProceedOauth(url: URL) -> Bool {
      ...
      guard url.pathComponents.map({ $0.lowercased() }).contains("callback") else {
         return false
      }
   ...
   static func didProceedNative(url: URL) -> Bool {
      for socialNetwork in [SocialNetwork.facebook, .google, .odnoklassniki, .vkontakte, .yandex] {
...
public extension SocialNetwork {
   ...
   var oauthUrl: URL {
      ...
      case .yandex:
         let path = "https://oauth.yandex.com/authorize"
         var parameters: [String: String] = [
            "client_id": clientId,
            "redirect_uri": urlRedirectSimplified,
            "response_type": oauthUsingCodeFlow ? "code" : "token",
            "state": state,
            "force_confirm": "1"
         ]
         if let scope = scope, scope.isEmpty == false {
...
extension SocialNetwork {
   ...
   func getUrlRequest(with code: String) -> URLRequest {
      ...
      case .vkontakte:
         ...
         return getUrlRequestFrom(path: path, parameters: parameters)
      case .yandex:
         let path = "https://oauth.yandex.com/token"
         let parameters = [
            "code": code,
            "client_id": clientId,
            "client_secret": clientSecret,
...
public extension SocialNetwork {
   ...
   func getToken(parameters: [String: String]) -> String? {
      switch self {
      case .facebook:
